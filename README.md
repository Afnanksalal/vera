# Vera: Mandate Claim Ledger

Vera reconciles purchases that an AI agent made on someone's behalf. When an agent buys under a mandate, the money arrives like any ordinary card payout, while the proof of what was authorized is scattered across an AP2 mandate, an ACP token, an x402 receipt, a processor settlement, and a bank credit. Vera puts that trail back together, proves or flags every step, and keeps a signed record that a stranger can re-check.

A language model does the investigating. A deterministic verifier re-derives every verdict from the raw rows, so a wrong or invented answer is rejected instead of booked. The model is useful; it is never trusted.

- Live console and demo: the `/ledger` page runs a full close in the browser.
- Command line: `npm run mandate` exposes every stage.
- HTTP API: `/api/investigate`, `/api/reconcile`, `/api/anomaly`, `/api/ledger`.

## Why this exists

A bank statement cannot tell an agent apart from a person. Both land as a payout. Matching a payout to an order on amount and date, the way most reconciliation still works, quietly accepts an overspent mandate, a missing receipt, a retried double charge, or a lump that mixes several customers. Those are the cases that cost a finance team real money, and they are exactly the ones a plain join misses.

Vera treats each agent sale as seven typed claims and closes the books only when each claim is proven or explicitly flagged. On top of that it adds three capabilities that a rule engine alone cannot provide: combinatorial matching for lumped payouts, a calibrated accept-or-abstain rule with a guaranteed error bound, and open-world detection of patterns that no fixed rule describes.
...
The verifier is the trust boundary. The investigator, the matcher, and the anomaly proposer can all be a language model, and none of them can commit a result the verifier has not independently reproduced.
...
1. Typed claims. Every sale is checked on seven fronts: the mandate authorized it, the signed cart matches the charge, a receipt exists, the payment is not a duplicate, the settlement equals the cart, the bank credit is tagged to the intent, and refunds follow policy. The investigator gathers evidence with tools and proposes a verdict; the verifier re-derives it.
2. Combinatorial matching. Real payouts are lumped, so one bank credit is the sum of several settlements within a tolerance. An exact subset-sum solver enumerates and checks groupings. The model proposes groupings from amounts, dates, and counterparty hints, and the solver confirms them. One planted group is larger than the solver's search cap, so only the model recovers it.
3. Conformal risk control. A fallible matcher produces a confidence score. Split-conformal calibration picks an acceptance threshold whose upper bound on the accepted-error rate stays at or below a target, then abstains on the rest and sends them to a human. The output is a match rate with a stated guarantee rather than a self-reported number.
4. Open-world anomaly synthesis. Some risks are patterns across many clean sales, such as one agent splitting spend into several carts just under its cap within a short window. A proposer writes a rule in a small, safe language; the rule is executed and checked for coverage and coherence; accepted findings go to human review and are never actioned automatically.
...
npm test           # unit and property tests for the engine
npm run mandate:eval
```

Optional: connect a model to enable the live investigator.

```bash
cp .env.example .env.local     # add ANTHROPIC_API_KEY or OPENAI_API_KEY
```

Without a key, the deterministic close, the matcher, the risk control, and the anomaly discovery all run, and the test suite passes. Only the live model panels need a key.
...
Keys are read from the environment only. Nothing secret is committed. See `.env.example`.
...
src/
  ...
  components/     UI components (shadcn/ui on Base UI) and the mascot art
