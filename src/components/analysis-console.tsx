...
export function AnalysisConsole({ aiConfigured }: { aiConfigured: boolean }) {
  ...
  return <div className="grid gap-6">
    <Panel><div><h2 className="text-lg font-semibold">Match settlements to bank deposits</h2><p className="mt-1 text-sm text-muted-foreground">Adjust how much amount and date variation Vera may consider when looking for a match. AI-proposed matches are accepted only after Vera verifies the amounts and dates.</p></div><div className="mt-5 grid gap-4 sm:grid-cols-2"><Field label="Amount difference allowed (₹)"><Input type="number" min={0} max={100} step="0.01" value={toleranceRupees} onChange={(e) => setToleranceRupees(e.target.value)}/></Field><Field label="Date range (days)"><Input type="number" min={0} max={30} value={windowDays} onChange={(e) => setWindowDays(e.target.value)}/></Field></div><div className="mt-5 flex flex-wrap gap-3">{aiConfigured ? <><Button disabled={pending} onClick={() => run(true)}>{pending ? "Reconciling…" : "Reconcile with AI"}</Button><Button variant="outline" disabled={pending} onClick={() => run(false)}>Run evidence-only</Button></> : <Button disabled={pending} onClick={() => run(false)}>{pending ? "Reconciling…" : "Reconcile payments"}</Button>}</div>{!aiConfigured ? <p className="mt-3 text-xs text-muted-foreground">Connect the AI investigator in Settings to get verified model suggestions alongside the evidence-only results.</p> : null}{error ? <Notice tone="error" role="alert" className="mt-3">{error}</Notice> : null}</Panel>
    {result ? <><section aria-label="Reconciliation summary" className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Metric label="Payments" value={result.source.sales}/><Metric label="Verified matches" value={result.reconciliation.matches.length} tone="good"/><Metric label="Needs review" value={needsReview} tone={needsReview ? "bad" : "default"}/><Metric label="Still processing" value={result.reconciliation.in_transit_unit_ids.length}/></section>
      ...
      <Disclosure title="Advanced confidence details" className="rounded-2xl border border-border bg-card" triggerClassName="p-5" panelClassName="border-t border-border p-5">{result.risk.calibration ? <div className="grid gap-3 sm:grid-cols-3"><Metric label="Labelled examples" value={result.risk.calibration_rows}/><Metric label="Automatically accepted" value={result.risk.accepted}/><Metric label="Estimated error ceiling" value={`${(result.risk.calibration.ub_error * 100).toFixed(1)}%`} detail={`${Math.round((1 - result.risk.calibration.delta) * 100)}% confidence`}/></div> : <p className="text-sm text-muted-foreground">No confidence estimate is available until real reviewed outcomes are imported below.</p>}<p className="mt-4 text-xs text-muted-foreground">Matching policy: {result.controls.policy}. Data source: {result.reconciliation.source}.</p></Disclosure>
    </> : null}
  </div>;
}
