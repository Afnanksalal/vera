...
export async function GET() { return handle(async () => Response.json(calibrationStatus((await requireUser()).id))); }
...
export async function DELETE() {
  return handle(async () => { await assertSameOriginIfCookie(); const user = await requireUser(); if (!rateLimit(`calibration:${user.id}`, 10, 60_000)) return Response.json({ error: "Calibration rate limit reached.", code: "rate_limited" }, { status: 429 }); clearCalibration(user.id); return Response.json({ ok: true }); });
}
