...
export default async function ClosesPage() {
  ...
  return <div className="grid gap-8">
    ...
    {closes.length === 0 ? <EmptyState icon={FileCheck2} title="No reports" /> : <div className="grid gap-3">{closes.map((close) => {
    ...
    <Disclosure title="Verify exported report" className="rounded-2xl border border-border bg-card" triggerClassName="p-5" panelClassName="grid gap-4 border-t border-border p-5"><BundleVerifier/><a className="w-fit text-sm font-medium text-brand hover:underline" href="/api/v1/public-key">Download public key</a></Disclosure>
  </div>;
}
