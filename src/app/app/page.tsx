...
export default async function AppHomePage() {
  ...
  return <div className="grid gap-8">
    ...
    <section aria-label="Latest check summary" className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Metric label="Imported records" value={events}/><Metric label="Checks passed" value={close?.summary.proven ?? 0} tone="good"/><Metric label="Needs attention" value={close?.summary.excepted ?? 0} tone="bad"/><Metric label="Missing evidence" value={close?.summary.abstained ?? 0}/></section>
    {open.length > 0 ? <Panel><div className="flex items-center justify-between gap-4"><div><h2 className="font-semibold">What needs attention</h2><p className="mt-1 text-sm text-muted-foreground">Start with the newest issues.</p></div><Link href="/app/review" className="text-sm font-medium text-brand hover:underline">View all</Link></div><ul className="mt-5 divide-y divide-border">{open.slice(0, 5).map((issue) => <li key={issue.id} className="flex items-start gap-3 py-3 first:pt-0 last:pb-0"><AlertCircle aria-hidden className="mt-0.5 size-4 shrink-0 text-bad"/><div className="min-w-0"><p className="text-sm font-medium">{friendlyClaim(issue.claim_type)}</p><p className="mt-0.5 text-sm text-muted-foreground">{friendlyCode(issue.code)} · Payment <span className="font-mono text-xs">{issue.sale_id}</span></p></div></li>)}</ul></Panel> : close ? <Panel className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="font-semibold">Latest report</h2><p className="mt-1 text-sm text-muted-foreground">Checked {close.summary.sales} payments on {formatDateTime(close.summary.created_at)}.</p></div><Link href={`/app/closes/${close.summary.id}`} className={cn(buttonVariants({ variant: "outline" }), "h-10")}>Open report</Link></Panel> : null}
    <details className="group rounded-2xl border border-border bg-card"><summary className="flex cursor-pointer list-none items-center gap-3 p-5 text-sm font-medium"><FileJson2 aria-hidden className="size-4 text-muted-foreground"/>Import records manually<span className="ml-auto text-xs font-normal text-muted-foreground group-open:hidden">AP2, ACP, or x402 JSON</span></summary><div className="border-t border-border p-5"><p className="mb-4 text-sm text-muted-foreground">Use this when records do not come through your Razorpay connection or integration API.</p><IngestForm /></div></details>
  </div>;
}
