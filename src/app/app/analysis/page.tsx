...
export default async function AnalysisPage() {
  ...
  return <div className="grid gap-8"><PageHeader title="Reconcile" description="Match processor settlements to bank deposits and find unusual activity across payments." />{records.length === 0 ? <EmptyState icon={BarChart3} title="Import records before reconciling" description="Connect Razorpay or import records from the Overview, then return here to match settlements and bank deposits." action={<Link href="/app" className={cn(buttonVariants(), "h-10")}>Go to Overview</Link>} /> : <><AnalysisConsole aiConfigured={ai.configured}/>{sales.length && ai.configured ? <AiInvestigator sales={sales} status={{ enabled: true, provider: ai.provider, name: ai.model }}/> : null}<CalibrationForm rows={calibrationStatus(user.id).rows}/></>}</div>;
}
