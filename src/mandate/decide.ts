...
type SettlementRow = { net_paise: number; settled_on: string };
...
export function decideClaim(world: World, sale: Sale, type: string): Decision {
  ...
  switch (type) {
    case "AUTHORIZED": {
      ...
      if (!sig.valid) return { action: "except", code: "MANDATE_OVERSPEND" };
    ...
    case "CART_BOUND": {
      ...
      if (!cartSig.valid || !cartSig.hash_match || !cart || !payment) {
    ...
    case "SETTLED": {
      ...
      if (!payment || !settlement) return { action: "except", code: "SETTLEMENT_DRIFT" };
      if (settlement.net_paise !== payment.amount_paise) {
    ...
    case "BANKED": {
      ...
      if (!payment || !settlement) return { action: "except", code: "CHANNEL_UNTAGGED" };
